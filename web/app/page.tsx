...
const I = { home: '⌂', heart: '♡', heartFull: '♥', recent: '↺', previous: '⏮', next: '⏭', play: '▶', pause: '❚❚', repeat: '↻', volume: '♫' };
...
export default function Home() {
  ...
  const step = (direction: number) => { const index = tracks.findIndex(track => track.id === currentId); selectTrack(tracks[(index + direction + tracks.length) % tracks.length]); };
  const favorite = (id: string) => setFavorites(items => save('qinsheng-favorites', items.includes(id) ? items.filter(item => item !== id) : [...items, id]));
  const nav: { id: View; label: string; icon: string }[] = [{ id:'home',label:'发现秦声',icon:I.home },{ id:'favorites',label:'我的收藏',icon:I.heart },{ id:'recent',label:'最近播放',icon:I.recent }];
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {view === 'home' && !query && <>
        ...
        <section className="moods"><button onClick={() => setQuery('秦腔')}><span>鼓</span><div><strong>秦腔真声</strong><small>听关中乡音</small></div></button><button onClick={() => setQuery('二胡')}><span>弦</span><div><strong>器乐秦韵</strong><small>弦上陕西风</small></div></button><button onClick={() => setQuery('CC')}><span>许</span><div><strong>公开授权</strong><small>来源清晰可试听</small></div></button></section>
      </>}
    ...
    <footer className="player"><audio ref={audioRef} src={current.audio} loop={repeat} onTimeUpdate={event => setProgress(event.currentTarget.currentTime)} onLoadedMetadata={event => setDuration(event.currentTarget.duration)} onEnded={() => !repeat && step(1)} />
      ...
      <div className="player-tools"><button className={repeat ? 'on' : ''} onClick={() => setRepeat(!repeat)} aria-label="循环播放">{I.repeat}</button><span>{I.volume}</span><input aria-label="音量" type="range" min="0" max="1" step=".01" value={volume} onChange={event => setVolume(Number(event.target.value))} style={{'--value':`${volume*100}%`} as React.CSSProperties}/></div>
    </footer>
  </main>;
}
